...
const libsignal = __importStar(require("node-libsignal-me"));
...
function signalStorage({ creds, keys }) {
    return {
        loadSession: async (id) => {
            ...
            // Return new empty session record if none exists
            return new libsignal.SessionRecord();
        },
        ...
        storeSenderKey: async (senderKeyName, key) => {
            const keyId = senderKeyName.toString();
            // Use proper binary serialization instead of JSON
            const serialized = key.serialize();
            await keys.set({ 'sender-key': { [keyId]: Buffer.from(serialized) } });
        },
