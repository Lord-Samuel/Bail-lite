...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, signalRepository, keyStore, logger, options, getMessage }) => {
    ...
    if (protocolMsg) {
        ...
        const participantsIncludesMe = () => {
            participants.find(p => {
                if (p.id) {
                    if (areJidsSameUser(meId, p.id)) return true;
                    if (meLid && areJidsSameUser(meLid, p.id)) return true;
                }
                if (p.phoneNumber && areJidsSameUser(meId, p.phoneNumber)) return true;
                if (p.lid && meLid && areJidsSameUser(meLid, p.lid)) return true;
                return false;
            })
        }
