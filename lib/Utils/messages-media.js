...
import { read as pixRead } from 'pixcore';
...
export const extractImageThumb = async (bufferOrFilePath, width = 32) => {
    ...
    const buffer = typeof bufferOrFilePath === 'string'
        ? await fs.readFile(bufferOrFilePath)
        : bufferOrFilePath
    
    const img = await pixRead(buffer)
    const metadata = await img.metadata()

    const resized = await img.resize(width, undefined, { fit: 'contain' })
    const jpegBuffer = await resized.toBuffer({ format: 'jpeg', quality: 80 })
...
export const generateProfilePicture = async (mediaUpload, dimensions) => {
    ...
    if (dimensions) {
        const resized = await img.resize(720, 720, { fit: 'contain' });
        const jpegBuffer = await resized.toBuffer({ format: 'jpeg', quality: 100 });
        return { img: jpegBuffer };
    } else {
        const resized = await img.resize(640, 640);
        const jpegBuffer = await resized.toBuffer({ format: 'jpeg', quality: 100 });
        return { img: jpegBuffer };
    }
};
