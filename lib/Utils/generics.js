...
const generateMessageID = (userId) => {
    ...
    const random = crypto_1.randomBytes(12) // 96 bits of randomness
    
    const data = `${timestamp}|${userId}|${random.toString('hex')}`
    const hash = crypto_1.createHash('sha256').update(data).digest('hex')
	// Use full hash for better uniqueness
    const genId =  `3A-BIXI${hash}`.substring(0, 19) // WhatsApp typically accepts up to 32 chars
    
    return genId.toUpperCase();
}
...
const generateDesktopMessageID = () => {
    ...
    return (prefix + random.toString('hex')).toUpperCase(); 
};
